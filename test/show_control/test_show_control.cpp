...
static void engine() {
  ...
  assert(a==65535 && b==0 && c==1); uint16_t no=777; eq(e.enqueue(one,now,no),"density"); assert(no==777 && e.depth()==3);
  ...
  for(unsigned i=0;i<1000;++i,++now) { auto t=e.next(now,true); if(t.kind==TxKind::Event) assert(delta(t.packet.event.executeAt,now)<=1200); e.sent(t,now,true); }
  ...
  for(unsigned i=0;i<3000;++i,++now) { auto t=e.next(now,true); if(t.kind==TxKind::Event) assert(t.packet.event.command==Command::Release && t.packet.event.executeAt==stopDue); e.sent(t,now,true); }
...
static void files() {
  ...
  uint32_t now=ready(e); assert(!p.play(e,now)); const uint32_t start=now; unsigned packets=0;
  for(;now<start+8500;++now) { p.tick(e,now); auto tx=e.next(now,true); if(tx.kind==TxKind::Event)++packets; e.sent(tx,now,true); assert(e.depth()<=3); assert(p.state()!=PlayerState::Error); }
  ...
  Player longPlayer; longPlayer.load(longShow.reader()); for(unsigned i=0;i<10000 && longPlayer.state()==PlayerState::Validating;++i)longPlayer.tick(e,0); assert(longPlayer.state()==PlayerState::Loaded && longPlayer.eventCount()==10000); assert(sizeof(Player)<600);
}
...
int main() { codec(); engine(); files(); requests(); puts("Show codec/clock/engine/player/parser/cache: PASS"); }
